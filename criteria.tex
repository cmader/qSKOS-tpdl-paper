...
The quality criteria we identified have in common that they are applied on a given SKOS vocabulary and identify a set of resources, which do not fulfill that criterion. Therefore, we can regard a quality criterion as being an abstract filtering function $F$ that takes a given vocabulary graph $V$ as input and returns a set of affected resources $IR$ \todo{CM}{think about it and try to formalize this.} Each quality criterion we found can be implemented as a realization of such an abstract filtering function. Since the return value is a set of resources, statistical analyses can be computed on the number of resources in that set.
...
\subsection{Labeling and Documentation Criteria}\todo{CM}{describe}

\subsubsection{Omitted Language Tags}\todo{CM}{describe}

\subsubsection{Incomplete Language Coverage}\todo{CM}{describe}

\subsubsection{Undocumented Concepts}\todo{CM}{describe}

\subsubsection{Potentially Semantically Related Concepts}\todo{CM}{describe}
