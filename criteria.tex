...
In the following, we explain the origins and design rationale for each quality issue and define the corresponding quality checking function. For better readability and due to lack of space we provide only semi-formal definitions and refer to the online documentation and the implementation source code for further details.
...
% Function definition
Omitted top concepts can be detected by iterating over all \texttt{skos:ConceptScheme}s and collecting those that do not occur in relations established by the properties \texttt{skos:hasTopConcept} or \texttt{skos:topConceptOf}.
