...
\todo{CM}{add TODOs for each of Antoine's comment.}
\todo{CM}{think how we could express this similar to OWL semantics}

\begin{definition}[SKOS Data Model] Let $R$ be the set of all resources, as defined in \cite{Jacobs2004} and $V = (C, A, LL, LR, SR, AR)$ be a controlled vocabulary expressed in SKOS, with
...
\begin{itemize}
	...
	\item \(LR \subseteq C \times LL\) being the set of \textbf{lexical relations} associating conceptual resources with lexical labels, e.g., via \texttt{rdfs:label} or subproperties thereof. \todo{BH}{koennten wir uns hier evtl sparen, brauchen das nur fuer Concept Documentation Coverage Ratio}
...
Let \(G = (C, SR \cup AR)\) \todo{CM+BH}{antoine: I'd argue against having one criteria for both sets of relations. Having a concept scheme will never compensate the fact that the concept has no relation (and vice versa). Similarly, the fact of not having a concept scheme will never be compensated by belonging to a skos:Collection.} and \(deg(c)\) be the degree, hence the number of in- and outgoing edges, of a node $c \in C$ in a graph $G$. We can then define loose concepts as a function \(lc : G \rightarrow \mathbb{N}_{0}\), with \[lc(g) = \left|\left\{c \in C : deg(c) = 0\right\}\right|\] \todo{CM+BH}{antoine argues for defining a ratio here}
